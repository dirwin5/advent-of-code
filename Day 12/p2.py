...
def unfold(line):
    ...
    record = line_split[0]
    record_unfolded = record
    for i in range(4):
        record_unfolded += f'?{record}'
    solution = line_split[1]
    solution_unfolded = solution
    for i in range(4):
        solution_unfolded += f',{solution}'

    return f'{record_unfolded} {solution_unfolded}'
...
total = 0
for k, line in enumerate(input_list):
    line = unfold(line)
    line_split = line.split()
    record = line_split[0]
    ...
    print(f'Line {k+1} of {len(input_list)} complete. - {len(true_perms)} arrangements')
