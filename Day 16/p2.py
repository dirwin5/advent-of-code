...
def main():
    ...
    # define all possible starting tiles and directions. List of tuples in format (Tile, beam direction)
    start_tile_list = []
    for x in range(max_x + 1):
        start_tile_list.append((TILE_DICT[x, 0], 'd'))
        start_tile_list.append((TILE_DICT[x, max_y], 'u'))
    for y in range(max_y + 1):
        start_tile_list.append((TILE_DICT[y, 0], 'r'))
        start_tile_list.append((TILE_DICT[y, max_x], 'l'))

    # iterate over every starting position
    max_total = 0
    for i, start_tile in enumerate(start_tile_list):
        tile_list = [start_tile]
        ...
        for tile in TILE_DICT.values():
            if tile.energised is True:
                total += 1
            # reset tile attributes for next iteration
            tile.energised = False
            tile.input_directions = []

        print(f'\nTotal for starting position {i+1}: {total}')
        if total > max_total:
            max_total = total
